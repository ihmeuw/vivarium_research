...
.. list-table:: Risk Effect Parameters for No ACS
  ...
  * - :math:`\text{RR}^\text{no ACS}`
    ...
    - N/A
    - Value to be used in sim for lack of access to ACS (i.e. for both :math:`\text{RR\_ACS | CPAP}` and :math:`\text{RR\_ACS | no CPAP}`, for more
      on why we have the assumption that :math:`\text{RR\_ACS | CPAP} =  \text{RR\_ACS | no CPAP}`, please see the limitations section below.)
  * - :math:`\text{RR}^\text{ACS}`
