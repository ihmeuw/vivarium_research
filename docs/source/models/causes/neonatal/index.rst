...
The above AMRs are population-level probabilities.
For our simulation, each simulant :math:`i` needs to be assigned an individual-level all-cause mortality risk :math:`\text{AMR}_i`, which we calculate as follows.
We begin with the population AMR and use the LBWSG PAF to derive a risk-deleted AMR to which we can then apply the relative risk of LBWSG matching any risk exposure level.  Mathematically this is achieved by the following formula:
