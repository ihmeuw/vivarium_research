...
households, so that there are collisions between residential
addresses.
...
module, but it doesn't have a reference, so I would also like to do some
research to check if it is right:
