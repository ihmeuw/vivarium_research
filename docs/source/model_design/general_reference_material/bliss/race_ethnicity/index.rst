...
===================================================
Inclusive Health Metrics: Race, Ethnicity, Ancestry
===================================================
...
an introduction to race, ethnicity, and ancestry (REA) from a scientific perspective,
a glossary of commonly used REA-related terms, recommendations for how to address REA-related limitations in external data
sources, discussion of the implications of imperfect proxy measures with regards to REA,
...
.. list-table:: **What this is vs. What this isn't**
   ...
   * - An accessible guide intended for everyone’s use, regardless of your familiarity with inclusive language practices. 
     - An intimidating guide that is full of jargon and can only be used by Diversity, Equity, and Inclusion (DEI) experts. 
...
reductive and racialized research practices can create. People from racially, ethnically, and ancestrally diverse backgrounds experience 
...
An accurate understanding of REA improves the validity of our research methods and conclusions about 
all people, not only racial, ethnic, and ancestral minority groups. One example of the harms from simplistic REA categorization is in our 
understanding of lung cancer rates. If we only use the overly homogenized racial group "Asian", rates of nonsmoking-related lung 
...
As human beings, we each have a lived experience of race, ethnicity, and ancestry—none of us are neutral outside observers. We have all 
unconsciously internalized ideas about REA from our experiences interacting with other people and with society. 
...
health outcomes. Despite their importance in
...
regarding REA. This requires carefully examining the data
...
to recognize the complexities of REA and to
...
but in the context of this guide we are focused on geographic and subpopulation ancestry.
...
Subpopulation ancestry is often the same as geographic ancestry,
but can be important in certain cases when distinct groups
existed in a geographic area that mostly had children with other members of the same group. 
...
Strictly speaking, this is *genealogical* (geographic/subpopulation) ancestry, since it counts ancestors equally.
On the other hand, *genetic* (geographic/subpopulation) ancestry refers to the ancestors you inherited genes from;
...
.. note::
  When people in such a group also primarily have children
  with others in the same group, the group can also be a
  subpopulation for ancestry purposes (see previous section).
...
.. list-table:: **Glossary**
   ...
   * - Ancestry
     - Broadly, characteristics of one’s ancestors. In this guide we refer to *geographic* ancestry, where your ancestors are from, or lived approximately 500 years ago;
       or *subpopulation* ancestry, a subpopulation your ancestors were part of that mostly had
...
and most of the research we have done thus far on REA
...
The US has an official Federal system for defining and measuring REA.
...
The key differences to note are that the 1997 standards have two separate questions,
while the 2024 standards have a single question.
...
Officially, despite the fact that the 1997 standards ask for "origins" in the text boxes
underneath each race checkbox,
and the fact that the 2024 standards ask for detailed terms like "Chinese" which could
easily be interpreted as ancestral categories,
these standards *only* cover race and ethnicity, *not* ancestry.
As such, the Census Bureau `continues to ask an additional question about ancestry <https://www.census.gov/acs/www/about/why-we-ask-each-question/ancestry/>`__
on the American Community Survey, pictured below.

.. image:: acs_ancestry_question.png

In cognitive interviews conducted by the Census
Bureau before the 2016 ACS Content Test, half of respondents expressed that they felt the race and ancestry questions were duplicative. [Westat_2014]_
80% of respondents in the Content Test answered the ancestry
question with completely or partially the same response they put in the Hispanic origin or race question. [Mills_2019]_
As a result of this confusion and significant non-response on the ancestry question,
the Census Bureau is considering `removing the ancestry question altogether <https://www2.census.gov/about/partners/cac/nac/meetings/2023-05/presentation-ancestry-and-race-2021-acs.pdf>`__.
...
concepts of REA fit into your research question is a critical
...
ethnicity and/or ancestry important to my research question?”, “What is gained by considering
race and/or ethnicity and/or ancestry in my research project?”, and “Is race, ethnicity, ancestry, or
some combination or sub-dimension the appropriate measure for my analysis?”. An
...
REA categories and associations will vary by population; you
should be sure to have an understanding of the dynamics of the population
relevant to your research and remember that REA categories used
...
There are three main challenges in considering REA in biomedical
...
expanded upon throughout the remainder of this guide.
    ...
    1.  To account for the limitations of REA data
    2.  To distinguish between race/ethnicity/ancestry as risk factor or as risk marker
    3.  To avoid contributing to the racial/ethnic/ancestral division of society

Finally, make sure to be aware of your target journal’s policies regarding REA.
Many journals have specific established guidelines that must be
adhered to.
...
is not only respectful and sensitive to the complexities of REA
...
For example, we might use a meta-analysis that cites individual population-based studies.

Frequently, upstream research does not report REA in ways that
...
REA variables in upstream research 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**We should not take the names of REA variables in input data at
face value.**
Due to prevalent misunderstandings of race, ethnicity, and ancestry as distinct and complex concepts,
...
**we should assume that it is a non-differentiated REA measure,**
unless there are clear signs of effort to construct a valid measure for race, ethnicity, or ancestry
(or a specific dimension of one of these).
Unfortunately, given current research practices, it is most common that a single variable
labeled either "race" or "ethnicity" actually represents a non-differentiated REA measure.
...
REA categories that won't match up perfectly. One option
is to take the "least common denominator" approach, which essentially means
using the most granular REA categories possible that still
capture the data fully.

Often this will end up being the US standard race and ethnicity categories above. We strongly
...
what there is to be gained from this analysis and if REA are
important to include if you plan to use fewer categories than the US race and ethnicity standard.
...
REA without stigmatizing, othering, or otherwise causing harm to historically marginalized 
REA groups. It is important to be aware of how we talk about race and racism, 
...
We often need to be very concise, and can’t surface all the complexity of REA every time we reference them. 
So, we take the following approach:

Somewhere in the paper/report, we explain in a fair amount of detail the racial and/or ethnic and/or ancestral categorizations we used,
why we used them that way,
the limitations of our data sources with respect to how they collected REA data,
the REA categories used in input data,
how we completed our analysis across different datasets, etc. 
For example, this could be in a methods appendix or a technical report that accompanies a PowerPoint presentation of results.
...
.. list-table:: Resources on race, ethnicity, and ancestry
  ...
  * - `Use of Race and Ethnicity in Biomedical Publication
    ...
    - Outlines guidelines to be used when race and ethnicity are
      addressed in biomedical publications
...
.. [Westat_2014]
    Cognitive Testing of the 2016 American Community Survey Content Test Items. Briefing Report for Round 1 Interviews. https://www.census.gov/content/dam/Census/library/working-papers/2016/acs/2016_Westat_02.pdf
