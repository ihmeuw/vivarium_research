...
the many details in each step make it difficult to see the overall structure.
...
These could be passed on the command line to the run command, with flags such as ``--force-step=<step_name>`` to force re-running a specific step, or ``--skip-step=<step_name>`` to skip a specific step.
There would be a ``--dry-run`` flag on the command to see what steps would be automatically executed without actually running them.