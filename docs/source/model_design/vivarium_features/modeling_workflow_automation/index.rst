...
3. Run the simulation (using the simulation environment) with the ``psimulate`` command (once for each location),
   ...
   We will also need to pass an output directory for the simulation results in the team directory, something like
...
5. Generate the final figures etc. for the client by executing the notebook in ``src/tutorial/results_processing``.
...
Furthermore, there would no longer be a need for model numbers or names, as all data including simulation results and artifacts would be versioned and branched
alongside the code using Git (details on this below).

The workflow would be specified in a Python file within the repository.
This is what the workflow would look like for the example project discussed above:

.. literalinclude:: workflow.py
   :language: python

A separate command would open up an interactive display of the workflow's directed acyclic graph (DAG), showing the dependencies between different steps.
This would live-reload as edits are saved to the workflow file.
Here is a mockup of how this might look.

.. image:: dag.drawio.png
    :height: 800px

This is a crucial feature because reasoning about the dependency structure from the textual representation is difficult.
The textual representation contains many details which make it difficult to see the overall structure.
A command that live-reloads the DAG visualization allows those making edits to quickly see their effects
and notice mistakes.

Git tracking of data files
++++++++++++++++++++++++++

As mentioned above, all inputs, intermediates (including artifacts), and outputs would be tracked within Git (i.e. would get committed and included in PRs,
...
If a change were made to the SQ-LNS intervention component (not included in the PAF sim),
...
If a refactor were made to any step that changed its code but not its outputs, *only* that step would re-run.

The cache would be **shared** between users, so if you checked out another person's branch and ran the workflow, nothing would happen if
they already ran the workflow, since nothing would have changed since their run.

There would be a manual override to force re-running a specific step, even if the cache thinks it already knows the output.
There would also be an override in the other direction, to mark that a step's output would not change with changes you have made.
These overrides would be shared between users as well.