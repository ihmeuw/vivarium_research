...
In addition to automating the workflow, this file also replaces some functions in the ``loader.py`` file
(all custom loading logic that depends on external processes, such as the PAF sim, is moved to a separate Python step),
