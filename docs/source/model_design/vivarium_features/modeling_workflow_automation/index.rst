...
As mentioned above, all inputs, intermediates (including artifacts), and outputs are tracked within Git (i.e. get committed and included in PRs,
you can switch versions with ``git checkout``, etc).
...
If it became a capacity problem to store all past versions of large files, we would run a tool (that we would develop internally) that would follow this pattern
...
This process only continues until the storage for large files is beneath a target size.
...
The cache is **shared** between users, so if you check out another person's branch and run the workflow, nothing will happen if
they already ran the workflow, since nothing has changed since their run.
...
These overrides will update the shared cache.